...
const capitalGainsData: CapitalGainsSummary = {
  stcg: { profits: 70200.88, losses: 1548.53 },
  ltcg: { profits: 5020, losses: 3050 }
};

export function fetchHoldings(): Promise<HoldingItem[]> {
  return new Promise((resolve) => {
    setTimeout(() => resolve(holdingsData), 600);
  });
}

export function fetchCapitalGains(): Promise<CapitalGainsSummary> {
  return new Promise((resolve) => {
    setTimeout(() => resolve(capitalGainsData), 600);
  });
}